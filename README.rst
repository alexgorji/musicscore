...
.XSDComplexType` or :obj:`~musicxml.xsd.XSDSimpleType`which correspond to xml schema elements ``complexType`` and ``simpleType``. The
documentation of this package covers only the information given in the musicxml schema file, including the structure of possible or
...
#. :obj:`musictree.voice.Voice` is the fourth layer of a musictree.
    ...
    * If a :obj:`musictree.chord.Chord` is added to its beats which has a quarter duration greater than the sum of beats' quarter
      durations, the remaining Chord can be accessed via ``left_over`` property.
