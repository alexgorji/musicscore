...
class Test(XMLTestCase):
    ...
    def test_1(self):
        ...
        for measure in self.score.get_children_by_type(TreeMeasure):
            for part in measure.get_children_by_type(TreePart):
                for voice in part.tree_part_voices:
                    xml_chords.append(voice.xml_chords)
        tree_chords = []
        for measure in self.score.get_children_by_type(TreeMeasure):
            for part in measure.get_children_by_type(TreePart):
                for voice in part.tree_part_voices:
                    tree_chords.append(voice.chords)
        expected = [[xml_chord.notes[0].quarter_duration for xml_chord in xml_chord_list] for xml_chord_list in xml_chords]
        actual = [[chord._notes[0].quarter_duration for chord in chord_list] for chord_list in tree_chords]
    ...
    def test_2(self):
        # staves
        sf_1 = SimpleFormat(quarter_durations=[4])
        sf_2 = SimpleFormat(quarter_durations=[4])
        ...
        for measure in self.score.get_children_by_type(TreeMeasure):
            for part in measure.get_children_by_type(TreePart):
                for staff in part.tree_part_staves.values():
                    xml_chords.append(staff.xml_chords)
        tree_chords = []
        for measure in self.score.get_children_by_type(TreeMeasure):
            for part in measure.get_children_by_type(TreePart):
                for staff in part.tree_part_staves.values():
                    tree_chords.append(staff.chords)
